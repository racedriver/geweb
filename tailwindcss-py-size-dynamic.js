...
module.exports = plugin(function ({theme, variants, e, addComponents}) {
  ...
  let themeData = theme('pySizeDynamic');
  let modifiers = Array.isArray(themeData) ? themeData : Object.keys(themeData)
  const screens = ['', ...theme('pySizeDynamicScreens', Object.keys(theme('screens', {})))]

  const utilities = fromPairs(
    modifiers.map(modifier => [
      nameClass('py', modifier + "-dynamic"),
      apply(dropWhile(modifiers, (it) => it !== modifier).slice(0, screens.length)
        .map((it, index) => `${screens[index] !== '' ? screens[index] + '\:' : ''}py-${it}`))
    ])
  );

  addComponents(utilities, ['responsive', ...variants('pySizeDynamic')]);
}, {
  theme: {
    pySizeDynamic: theme => theme('padding'),
  },
  variants: {
    pySizeDynamic: ['responsive'],
  },
});
